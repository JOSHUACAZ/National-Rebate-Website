const APP_BUILD_VERSION = '2026.08.26.network.9';
...
function trackEvent(name,params={}){if(typeof window.gtag!=='function')return;window.gtag('event',name,params)}
function analyticsSnapshot(){const models=parseModels();const results=lastResults||{};const paid=Object.values(results).filter(r=>Number(r?.amount||0)>0);const total=Object.values(results).reduce((sum,r)=>sum+Number(r?.amount||0),0);return{dealer_network:selectedNetwork||'unselected',package_size:models.length,qualifying_rebate_count:paid.length,total_rebate_amount:total}}
